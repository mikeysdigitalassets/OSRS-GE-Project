...
const Table = ({ itemDetails, userId, triggerRerender }) => {
  ...
  const toggleWatchlist = async (item) => {
    ...
    if (isInWatchlist) {
      ...
      try {
        await axios.delete(`http://my-backend-env.eba-tqzpmtwd.us-east-1.elasticbeanstalk.com/api/user/${userId}/watchlist`, { data: { itemId: item.id } });
      ...
      try {
        await axios.post(`http://my-backend-env.eba-tqzpmtwd.us-east-1.elasticbeanstalk.com/api/user/${userId}/watchlist`, { itemId: item.id, itemName: item.name });
